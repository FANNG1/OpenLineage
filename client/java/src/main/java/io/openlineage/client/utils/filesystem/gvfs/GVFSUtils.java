...
public class GVFSUtils {
  ...
  public static boolean isGVFS(URI uri) {
    return GVFSFilesystemDatasetExtractor.SCHEME.equalsIgnoreCase(
        uri.getScheme());
  }

  public static void injectGVFSFacets(OpenLineage openLineage,
      DatasetCompositeFacetsBuilder builder, URI location) {
    // add GVFS data type
    builder.getFacets().datasetType(
        openLineage.newDatasetTypeDatasetFacet("Fileset", ""));
    // add location
    LocationDatasetFacet locationDatasetFacet = GravitinoFacets.newLocationDatasetFact(
        GVFSUtils.getGVFSLocation(location));
    builder.getFacets().put("fileset-location", locationDatasetFacet);
  }

  public static OutputDataset injectGVFSFacets(OpenLineage openLineage, OutputDataset outputDataset,
      URI location) {
    ...
    builder.getFacets().columnLineage(datasetFacets.getColumnLineage())
        .dataSource(datasetFacets.getDataSource()).tags(datasetFacets.getTags())
        .documentation(datasetFacets.getDocumentation())
        .lifecycleStateChange(datasetFacets.getLifecycleStateChange())
        .ownership(datasetFacets.getOwnership()).schema(datasetFacets.getSchema())
    ...
    return openLineage.newOutputDatasetBuilder()
        ...
        .outputFacets(outputDataset.getOutputFacets()).build();
  }
